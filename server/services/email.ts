  ...
  async sendEmail(to: string | string[], subject: string, text: string, html?: string) {
    try {
      const info = await this.transporter.sendMail({
        from: `"DefenseHub Security" <${process.env.SMTP_USER}>`,
...
Accounts Receivable`,
  ...
  async sendPhishingSimulation(to: string, scenario: any) {
    ...
    const mailOptions = {
      ...
      headers: {
        'X-Priority': '3',
        'X-DefenseHub-Type': 'phishing-simulation',
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 20px;">
          ...
          <p><strong>DefenseHub Security Team</strong></p>
        </div>
      </div>
    `;
